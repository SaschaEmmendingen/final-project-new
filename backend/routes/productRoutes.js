...
router.post('/bulk', async (req, res) => {
  try {
    ...
    if (!Array.isArray(products) || products.length === 0) {
      return res.status(400).json({ error: 'Body muss ein Array von Produkten sein.' });
    }

    // Überprüfe, ob alle Produkte das imageUrl-Feld enthalten
    for (const product of products) {
      if (!product.name || !product.description || !product.price || !product.category || !product.stock || !product.imageUrl) {
        return res.status(400).json({ error: 'Alle Produkte müssen alle erforderlichen Felder enthalten.' });
      }
    }
