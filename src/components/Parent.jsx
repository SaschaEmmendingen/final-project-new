...
import { useCart } from './CartContext'; // Importiere useCart
  ...
  return (
    <div className>
      <nav className="bg-white w-[100] py-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-4 items-center">
            ...
            <div className="flex justify-center items-center lg:justify-start lg:col-span-1 lg-row-span-2">
              <Link to="/" className="flex items-center">
               <img src={canvaSVG} alt="Logo" className="h-20 w-auto items-center" /> 
              </Link>
            </div>
            ...
            <div className="flex justify-center lg:justify-end lg:col-span-1 lg:space-x-10 relative ">
              <Link to="/kontakt" className="flex flex-col items-center text-black hover:text-gray-700">
                <FaPhone className="text-lg text-black-700" />
                <span className="mt-3 text-sm">Kontakt</span>
              </Link>
              <Link to="/konto" className="flex flex-col items-center text-black hover:text-gray-700">
                <FaUser className="text-lg" />
                <span className="mt-3 text-sm">Konto</span>
              </Link>
              <Link to="/warenkorb" className="flex flex-col items-center text-black hover:text-gray-700">
                <FaShoppingCart className="text-lg" />
                {cartItems.length > 0 && (
                  <span className="absolute top-0 right-0 bg-red-600 text-white text-xs rounded-full w-4 h-4 flex items-center justify-center -translate-x-1/2 -translate-y-1/2">{cartItems.length}</span>
                )}
        ...
        {/* Hamburger Button */}
        <button
          className={`lg:hidden fixed top-4 right-6 z-50 flex flex-col justify-between items-center w-10 h-6 ${isMenuOpen ? 'open' : ''}`}
          ...
          {isMenuOpen && (
            ...
            >
              ...
              <div className="flex space-x-8 mt-8">
                <Link to="/kontakt" className="text-white hover:text-gray-400 flex flex-col items-center" onClick={handleLinkClick}>
                  <FaPhone className="text-3xl" />
                  <span className="mt-2 text-sm">Kontakt</span>
                </Link>
                <Link to="/konto" className="text-white hover:text-gray-400 flex flex-col items-center" onClick={handleLinkClick}>
                  <FaUser className="text-3xl" />
                  <span className="mt-2 text-sm">Konto</span>
                </Link>
                <Link to="/warenkorb" className="text-white hover:text-gray-400 flex flex-col items-center relative" onClick={handleLinkClick}>
